...
namespace RayFire
{
    ...
    public class RFPhysic
    {
        ...
        public void GlobalReset()
        {
            ...
            {
                if (meshCollider is MeshCollider)
                {
                    (meshCollider as MeshCollider).convex         = false;
                    (meshCollider as MeshCollider).sharedMesh     = null;
                    (meshCollider as MeshCollider).sharedMaterial = null;
                }
            }
        }
        ...
        public static void SetDensity(RayfireRigid scr)
        {
            ...
            {
                scr.physics.rigidBody.SetDensity(RayfireMan.inst.materialPresets.Density(scr.physics.mt));
        ...
        {
            scr.physics.rigidBody.drag        = RayfireMan.inst.materialPresets.Drag(scr.physics.mt);
            scr.physics.rigidBody.angularDrag = RayfireMan.inst.materialPresets.AngularDrag(scr.physics.mt);
        }
        ...
        public static void SetFragmentsVelocity (RayfireRigid scr)
        {
            ...
            // Current velocity
            if (scr.meshDemolition.ch.wasUsed == true && scr.meshDemolition.ch.skp == false)
            {
                for (int i = 0; i < scr.fragments.Count; i++)
                    if (scr.fragments[i] != null)
                        scr.fragments[i].physics.rigidBody.velocity = scr.physics.rigidBody.GetPointVelocity (scr.fragments[i].transForm.position) * scr.physics.dm;
            }
        ...
        {
            // Custom collider
            scr.physics.ct = scr.meshDemolition.prp.col;
            
            // Size filter check
            if (scr.meshDemolition.prp.szF > 0)
                if (mesh.bounds.size.magnitude < scr.meshDemolition.prp.szF)
        ...
        public static void SetRigidCollider (RayfireRigid scr, Mesh mesh = null)
        {
            ...
            if (scr.physics.meshCollider == null)
            {
                ...
                if (scr.physics.ct == RFColliderType.Mesh)
                {
                    ...
                    if (scr.physics.pc == true && scr.meshFilter.sharedMesh.vertexCount < RayfireMan.coplanarVertLimit)
                    {
        ...
        {
            Collider col;
            scr.physics.clusterColliders = new List<Collider>(scr.fragments.Count);
            for (int i = 0; i < scr.fragments.Count; i++)
            {
        ...
        public static void CollectClusterColliders (RayfireRigid scr, RFCluster cluster)
        {
            ...
            if (scr.physics.clusterColliders == null)
                scr.physics.clusterColliders = new List<Collider>(cluster.shards.Count);
            else
        ...
        public PhysicMaterial PhysMaterial
        {
            get
            {
                ...
                // Crete new material
                return RFMaterialPresets.Material(mt);
            }
        }
    }
}
